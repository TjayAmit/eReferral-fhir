  ...
  try {
    ...
    if (!body.resourceType || body.resourceType !== 'Observation') {
      return NextResponse.json(
        { error: 'Resource must be an Observation' },
        { status: 400 }
      );
    }

    // Validate category for vital-signs
    if (!body.category || !Array.isArray(body.category)) {
      return NextResponse.json(
        { error: 'Observation must have a category (e.g., vital-signs)' },
        { status: 400 }
      );
    }

    const result = await fhirPost('Observation', body);
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = 'Observation?_count=100';
    if (patient) {
