...
const HL7_VALUESETS = [
  ...
  {
    id: "0.07",
    name: "Contact Point System / Use",
    code: "contact-point-system",
    url: "http://hl7.org/fhir/ValueSet/contact-point-system",
  },
];

export default function HL7ValueSetsPage() {
  const [selectedValueSet, setSelectedValueSet] = useState<typeof HL7_VALUESETS[0] | null>(null);
  const [expansion, setExpansion] = useState<any>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [filter, setFilter] = useState("");

  async function loadValueSet(valueSet: typeof HL7_VALUESETS[0]) {
  ...
  return (
    <>
      <AppPageHeader
        items={[
          { label: "Home", href: "/" },
          { label: "ValueSets", href: "/valuesets" },
          { label: "HL7 ValueSets" },
        ]}
        title="HL7 ValueSets"
        actions={
          <span className="muted">{HL7_VALUESETS.length} value set(s)</span>
        }
      />
        ...
        <div className="card">
          ...
          <div className="field">
            ...
            <select
              ...
              onChange={(e) => {
                const vs = HL7_VALUESETS.find((v) => v.id === e.target.value);
                if (vs) loadValueSet(vs);
              }}
            >
              <option value="">— select a value set —</option>
              {HL7_VALUESETS.map((vs) => (
