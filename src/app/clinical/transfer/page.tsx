...
export default function ClinicalTransferPage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/clinical-transfer?organization=${encodeURIComponent(orgId)}`, {
        headers: { "X-FHIR-Base-Url": baseUrl },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to load transfer patients");
  ...
  if (!orgId) {
    return (
      <>
        <AppPageHeader items={[{ label: "Home", href: "/" }, { label: "Clinical Transfer" }]} title="Clinical Transfer" />
        <div className="alert err">
          No organization linked to your account — Clinical Transfer lists patients
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      header: "Actions",
      cell: (info: any) => (
        <button onClick={(e: React.MouseEvent) => { e.stopPropagation(); router.push(`/clinical/transfer/${info.row.original.encounter.id}`); }}>
          View Details
        </button>
      ),
    },
  ...
  return (
    <>
      <AppPageHeader
        items={[{ label: "Home", href: "/" }, { label: "Clinical Transfer" }]}
              ...
              <tbody>
                {pageRows.map((row) => (
                  <tr
                    ...
                    className="clickable"
                    onClick={() => router.push(`/clinical/transfer/${row.original.encounter.id}`)}
                  >
