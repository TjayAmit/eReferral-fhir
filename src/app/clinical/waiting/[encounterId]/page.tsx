...
export default function ClinicalWaitingViewPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      const bundle = buildClinicalBundle({
        ...
        practitionerId: String((user as any)?.practitionerId || ""),
  ...
  async function handleDisposition(choice: Disposition) {
    if (!detail?.encounter || !detail?.patient?.id) return;
    if (choice === "discharge") {
      if (!window.confirm("Discharge — finish the visit and send the patient home?")) return;
    }
    setDisposing(choice);
    setDispError(null);
    setNotice(null);
    try {
      ...
      const { er } = applyDisposition(detail.encounter, detail.patient.id, choice, new Date().toISOString());
      ...
      setNotice(`Encounter finalized — ${choice}.`);
      load();
    } catch (err) {
                        ...
                        <div key={sr.id} style={{ padding: 12, background: "var(--surface)", borderRadius: 8, border: "1px solid var(--border)" }}>
                          ...
                          <dl className="kv" style={{ margin: 0 }}>
                            {sr.category?.[0]?.coding?.[0]?.display && (
                              <><dt>Category</dt><dd>{sr.category[0].coding[0].display}</dd></>
                            )}
                            {sr.code?.text && (
                              <><dt>Code</dt><dd>{sr.code.text}</dd></>
                            )}
                            {sr.authoredOn && (
                              <><dt>Authored</dt><dd>{new Date(sr.authoredOn).toLocaleString()}</dd></>
                            )}
                            {sr.requester?.reference && (
                              <><dt>Requester</dt><dd><code>{sr.requester.reference}</code></dd></>
                            )}
                            {sr.performer?.[0]?.reference && (
                              <><dt>Performer</dt><dd><code>{sr.performer[0].reference}</code></dd></>
                            )}
                            {sr.reasonCode?.[0]?.text && (
                              <><dt>Reason</dt><dd>{sr.reasonCode[0].text}</dd></>
                            )}
                            {sr.note?.[0]?.text && (
                              <><dt>Note</dt><dd>{sr.note[0].text}</dd></>
                            )}
                          </dl>
                        </div>
                      ))}
